...
async function GetPost(id: number): Promise<Array<any>> {
...
async function GetSeries(id: number): Promise<Array<any>> {
	return new Promise(async (res, rej) => {
		let Data,
			scheme = 'https' + '://' + 'api' + '.usamin' + '.cc/';
		try {
...
export async function getId(id: number): Promise<Array<any>> {
